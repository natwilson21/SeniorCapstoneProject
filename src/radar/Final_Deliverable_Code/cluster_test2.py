...
# Example usage of loading the model and making predictions
    ...
    if H >1:
        ...
        url = f"http://{server_url}:80/set_radar_room"  
        response = requests.post(url, json={"radar": True})
        print("Response:", response.json())
        dbscan.plot_clusters()

    else:
        print("Room Clear")
        url2 = f"http://{server_url}:80/set_radar_room"  
        response = requests.post(url2, json={"radar": False})
        print("Response:", response.json())
        dbscan.plot_clusters()
